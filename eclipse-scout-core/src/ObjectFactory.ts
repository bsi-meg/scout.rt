 ...
 */
import {BaseDoEntity, Constructor, FullModelOf, Id, InitModelOf, ModelOf, ObjectModel, objects, scout, TypeDescriptor, TypeDescriptorOptions} from './index';
import $ from 'jquery';
...
export class ObjectFactory {
  ...
  protected _ensureObjectType(scoutObject: any) {
    const isIdOrDataObject = scoutObject instanceof BaseDoEntity || scoutObject instanceof Id;
    return !isIdOrDataObject; // don't create objectType attribute for IDs and DOs
  }

  protected _ensureUniqueId(scoutObject: any, options?: ObjectFactoryOptions): boolean {
    const isIdOrDataObject = scoutObject instanceof BaseDoEntity || scoutObject instanceof Id;
    return scout.nvl(options.ensureUniqueId, !isIdOrDataObject /* don't create unique IDs for DOs and IDs by default */);
  }
   ...
   */
  getSubClassesOf<T extends object>(baseClass: Constructor<T>): Constructor<T>[] {
