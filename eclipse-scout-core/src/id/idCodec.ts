...
export const idCodec = {
   ...
   * @returns the parsed id or null if there is no value.
   */
  parse<TTypeName extends string>(rawId: string, typeNameProvider?: () => TTypeName): Id<TTypeName> {
   ...
   * @returns the parsed id or null if there is no value.
   */
  fromUnqualified<TTypeName extends string>(value: string, typeName: TTypeName): Id<TTypeName> {
   ...
   * @returns the parsed id or null if there is no value.
   */
  fromQualified<TTypeName extends string>(qualifiedId: string): Id<TTypeName> {
  ...
  /**
   * @returns the given id converted to its unqualified string representation. Unqualified means only value and signature are part of the result, the typeName is omitted. Returns null if there is no input.
   */
  toUnqualified(id: Id): string {
    if (!id) {
      return null;
    }
  ...
  /**
   * @returns the given id converted to its qualified string representation including typeName, value and signature. Returns null if there is no input.
   */
  toQualified(id: Id): string {
    if (!id) {
      return null;
    }
...
export interface Id<TIdTypeName extends string = string> {
  /**
