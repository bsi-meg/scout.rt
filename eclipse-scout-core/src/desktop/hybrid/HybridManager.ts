 ...
 */
import {
  DoEntity, Event, EventHandler, EventListener, EventMapOf, Form, HybridActionContextElements, HybridActionEvent, HybridManagerEventMap, HybridManagerWidgetAddEvent, HybridManagerWidgetRemoveEvent, InitModelOf, ObjectOrChildModel, Session,
  UuidPool, Widget
} from '../../index';
...
export class HybridManager extends Widget {
  ...
  /** @internal */
  onHybridEvent(id: string, eventType: string, data: DoEntity, contextElements: HybridActionContextElements) {
    this.trigger(`${eventType}:${id}`, {data, contextElements});
  }

  /** @internal */
  onHybridWidgetEvent(id: string, eventType: string, data: DoEntity) {
  ...
  protected _onHybridWidgetEvent(widget: Widget, eventType: string, data: DoEntity) {
    widget.trigger(eventType, {data});
  }

  protected _onHybridFormEvent(form: Form, eventType: string, data: DoEntity) {
    if (eventType === 'reset') {
      form.setData(data);
      form.trigger('reset');
   ...
   */
  callAction(actionType: string, data?: DoEntity, contextElements?: HybridActionContextElements): string {
   ...
   */
  callActionAndWait(actionType: string, data?: DoEntity, contextElements?: HybridActionContextElements): JQuery.Promise<DoEntity> {
   ...
   * @see AbstractHybridAction.fireHybridActionEndEvent
   */
  callActionAndWaitWithContext(actionType: string, data?: DoEntity, contextElements?: HybridActionContextElements): JQuery.Promise<HybridManagerActionEndEventResult> {
   ...
   */
  openForm(modelVariant: string, data?: DoEntity): JQuery.Promise<Form> {
   ...
   * @returns a promise that will be resolved once the form has been created
   */
  createForm(modelVariant: string, data?: DoEntity): JQuery.Promise<Form> {
