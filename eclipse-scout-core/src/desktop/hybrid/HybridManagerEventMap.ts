...
import {DoEntity, Event, HybridActionContextElements, HybridManager, PropertyChangeEvent, Widget, WidgetEventMap} from '../../index';

export interface HybridActionEvent<TObject extends DoEntity = DoEntity, T = HybridManager> extends Event<T> {
  data: {
    id: string;
    actionType: string;
    data: TObject;
...
export interface HybridActionEndEvent<TObject extends DoEntity = DoEntity, T = HybridManager> extends Event<T> {
  data: TObject;
  contextElements: HybridActionContextElements;
}
