 ...
 */
import {Constructor, dataObjects, DoRegistry, DoValueMetaData, doValueMetaData, ObjectFactory, objects} from '../../index';
...
export class DataObjectSerializer {
  ...
  protected _writeJsonType(target: any, constructor: Constructor) {
    ...
    if (objectType) {
      const typeName = DoRegistry.get().toTypeName(objectType);
      if (typeName) {
