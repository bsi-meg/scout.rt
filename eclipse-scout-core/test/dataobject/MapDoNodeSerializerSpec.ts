...
import {BaseDoEntity, dataObjects, dates, DoRegistry, Id, IdParser, NumberId, ObjectFactory, scout, StringId, typeName, UuId} from '../../src/index';
...
describe('MapDoNodeSerializer', () => {
  ...
  beforeAll(() => {
    ObjectFactory.get().registerNamespace('scout', {MapFixture01Do, MapFixture02Do, MapFixture03Do}, {allowedReplacements: ['scout.MapFixture01Do', 'scout.MapFixture02Do', 'scout.MapFixture03Do']});
    const doRegistry = DoRegistry.get();
    doRegistry.add(MapFixture01Do);
    doRegistry.add(MapFixture02Do);
    doRegistry.add(MapFixture03Do);
  });

  afterAll(() => {
    const doRegistry = DoRegistry.get();
    doRegistry.remove(MapFixture03Do);
    doRegistry.remove(MapFixture02Do);
    doRegistry.remove(MapFixture01Do);
  });
  ...
  it('map keys are handles correctly', () => {
    const map = scout.create(MapFixture03Do);
    map.testMap.set(StringId.of('val1', 'Test'), 'val11');
    map.testMap.set(StringId.of('val2', 'Test'), 'val22');
    map.testMap2.set(NumberId.of(33, 'TestNumId'), 333);
    map.testMap2.set(UuId.of('44', 'TestUuId'), 444);

    // explicitly register these typeNames. Otherwise, the deserializer cannot know which class it would be (because in the code only 'Id' exists).
    IdParser.register('TestNumId', NumberId);
    IdParser.register('TestUuId', UuId);

    const serialized = dataObjects.serialize(map);
    expect(JSON.stringify(serialized)).toBe(JSON.stringify(JSON.parse(`{
        "_type": "MapFixture03",
        "testMap": {
          "Test:val1": "val11",
          "Test:val2": "val22"
    ...
    const deserialized = dataObjects.deserialize(serialized) as MapFixture03Do;
    expect(deserialized).toBeInstanceOf(MapFixture03Do);
    expect(deserialized.testMap).toBeInstanceOf(Map);
    expect(deserialized.testMap.size).toBe(2);
    expect(deserialized.testMap.keys().next().value).toBeInstanceOf(StringId);
    expect(deserialized.testMap2).toBeInstanceOf(Map);
    expect(deserialized.testMap2.size).toBe(2);
    let it = deserialized.testMap2.keys();
    expect(it.next().value).toBeInstanceOf(NumberId);
    expect(it.next().value).toBeInstanceOf(UuId);

    expect(deserialized.equals(map)).toBeTrue();

    IdParser.remove('TestNumId');
    IdParser.remove('TestUuId');
  });
});
...
@typeName('MapFixture03')
export class MapFixture03Do extends BaseDoEntity {
  testMap: Map<StringId<'Test'>, string>;
  testMap2: Map<Id<any>, number>;

  constructor() {
    super();
    this.testMap = new Map();
    this.testMap2 = new Map();
  }
}
