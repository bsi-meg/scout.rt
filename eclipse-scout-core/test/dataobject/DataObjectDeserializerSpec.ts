 ...
 */
import {BaseDoEntity, DataObjectDeserializer, dataObjects, dates, DoRegistry, ObjectFactory, typeName} from '../../src/index';
...
describe('DataObjectDeserializer', () => {
  ...
  afterAll(() => {
    const doRegistry = DoRegistry.get();
    doRegistry.remove(Fixture01Do);
    doRegistry.remove(Fixture02Do);
    doRegistry.remove(Fixture03Do);
    doRegistry.remove(Fixture03SubDo);
  });
