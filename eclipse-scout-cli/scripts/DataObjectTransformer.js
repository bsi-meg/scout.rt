...
module.exports = class DataObjectTransformer {
  ...
  transform(node) {
    if (ts.isSourceFile(node)) {
      ...
      if (this.doRegistryAddStatements.length) {
        // add auto-register DO statements to the source file end
        const statements = [...transformedFile.statements, ...this.doRegistryAddStatements];
        ...
        this.doRegistryAddStatements = []; // clear added statements
      }
    ...
    if (ts.isClassDeclaration(node)) {
      ...
      if (typeNameDecorator) {
        // it is a data object: remember DoRegistry.add statement to add at the end to the source file
        this.doRegistryAddStatements.push(this._createRegistryAddStatement(node.localSymbol.escapedName));
   ...
   * 2. Resolves the Scout JS namespace for that module.
   *
   * @param typeNode {ts.TypeReferenceNode} The type reference for which the namespace it is declared in should be resolved.
   * @returns {string|null} The namespace of the module that contains the given type.
   */
  ...
  /**
   * Creates an ExpressionStatement calling DoRegistry.add for the given data object name.
   * Creates the following call:
   * <pre>
   * window['scout']['DoRegistry'].get().add(className);
   * </pre>
   * @param className {string} The local class name of the data object.
   * @returns {ts.ExpressionStatement}
   */
  _createRegistryAddStatement(className) {
    if (!className) {
      throw new Error('DoRegistry.add not supported for anonymous data objects.');
    }
    ...
    const doRegistry = ts.factory.createElementAccessExpression(scout, ts.factory.createStringLiteral('DoRegistry'));
    const get = ts.factory.createPropertyAccessExpression(doRegistry, 'get');
    ...
    const addCall = ts.factory.createCallExpression(add, undefined, [ts.factory.createIdentifier(className)]);
